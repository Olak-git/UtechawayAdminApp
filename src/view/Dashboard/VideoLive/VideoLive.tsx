...
import { clearMeeting, setLiveMeetingId, setLiveSharedScreen } from "../../../feature/meeting.slice";
...
const VideoLive: React.FC<{navigation: any}> = ({navigation}) => {
    ...
    const [url, setUrl] = useState('https://utechaway.com/live')
    ...
    return !fetchLoading ? (
        <SafeAreaView style={tw`flex-1`}>
            <StatusBar hidden />
            <DashboardHeaderSimple navigation={navigation} title='Meeting' containerStyle={[{backgroundColor: CodeColor.code1}]} />
            <WebView
                source={{uri: url}}
                cacheEnabled
                scrollEnabled
