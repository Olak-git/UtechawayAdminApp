...
const Gifted: React.FC<GiftedProps> = ({endFetch, state, messages=[], setMessages, inputs, handleOnChange=(x:string,v:any)=>{}, handleFilePicker=()=>{}, handleOnSubmit, user}) => {
    ...
    const onSend = useCallback((messages = []) => {
        ...
        dispatch(addMessages({[user.slug]: messages}));
    }, [inputs])
    ...
    const renderChatEmpty = () => {
        return (
            <View style={[tw`flex-1 justify-end items-center pb-10`, {transform: [{rotateX: '180deg'}]}]}>
                ...
                />
                <Text style={[tw`text-black px-2`, {fontFamily: 'YanoneKaffeesatz-Regular'}]}>{user.prenom + ' ' + user.nom}</Text>
            </View>
        )
    }
        ...
        return (
            <Send {...props} containerStyle={[tw`ml-0 self-center justify-center items-center p-1`, {height: 40}]}>
                ...
                <Image
                    ...
                    PlaceholderContent={<Text style={tw`text-center text-xs text-gray-600`}>Envoyer</Text>}
        ...
        return (
            ...
            ?
                <View style={[tw``,{marginBottom: (layoutHeight > 0 ? layoutHeight - 22 : layoutHeight)}]}>
                    <Text style={tw`text-black text-right pr-3`}>transfert en cours...</Text>
                </View>
            :
                !endFetch 
    ...
    const renderInputToolbar = (props: any) => {
        ...
        if(user.del == 1) {
            return <Text style={tw`text-white text-center mt-2`}>Compte utilisateur supprimé</Text>
        }
